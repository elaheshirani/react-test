        ...
        return(
            <div className="container mt-5 mb-5">
                <div className="row justify-content-md-center">
                    ...
                    <div className="col-md-6 signDiv">
                        <div className="header pt-3  peach-gradient bg-reg" >Edit Information</div>
                        <form className="signForm" onSubmit={this.handleSubmit.bind(this)}>
                            <div className="form-group ">
                                <label>Name</label>
                                <input type="text" className={["form-control" , errors["name"] ? 'is-invalid' : ''].join(' ')} id="name" name="name" placeholder="Name" onChange={this.handleChange.bind(this)} value={this.state.userInfo.name}/>
                                <span className="invalid-feedback rtl" style={{ display : errors["name"] ? 'block' : 'none'}}>{errors["name"]}</span>
                            </div>
                            <div className="form-group ">
                                <label>Family</label>
                                <input type="text" className={["form-control" , errors["family"] ? 'is-invalid' : ''].join(' ')} id="family" name="family" placeholder="Family" onChange={this.handleChange.bind(this)} value={this.state.userInfo.family}/>
                                <span className="invalid-feedback rtl" style={{ display : errors["family"] ? 'block' : 'none'}}>{errors["family"]}</span>
                            </div>
                            <div className="form-group ">
                                <label>Email</label>
                                <input type="text" className={["form-control" , errors["email"] ? 'is-invalid' : ''].join(' ')} id="email" name="email" placeholder="Email" onChange={this.handleChange.bind(this)} value={this.state.userInfo.email}/>
                                <span className="invalid-feedback rtl" style={{ display : errors["email"] ? 'block' : 'none'}}>{errors["email"]}</span>
                            </div>
                            <div className="form-group ">
                                <label>Mobile</label>
                                <input type="text" className={["form-control" , errors["mobile"] ? 'is-invalid' : ''].join(' ')} id="mobile" name="mobile" placeholder="Mobile" onChange={this.handleChange.bind(this)} value={this.state.userInfo.mobile}/>
                                <span className="invalid-feedback rtl" style={{ display : errors["mobile"] ? 'block' : 'none'}}>{errors["mobile"]}</span>
                            </div>
                            <div className="form-group">
                                <label>Birthday</label>
                                <DayPickerInput className={["form-control" , errors["birthday"] ? 'is-invalid' : ''].join(' ')} inputProps={{name: 'birthday'}} placeholder='Birthday' onDayChange={this.handleChangeBirthday.bind(this)} value={this.state.userInfo.birthday.substr(0,10)}/>
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-sm-10">
                                        <div className="form-check">
                                            <input className="form-check-input" type="radio" name="gender" id="male" value="1"  onChange={this.handleChange.bind(this)} checked={this.state.userInfo.gender === '1'}/>
                                            <label className="form-check-label" htmlFor="male">
                                                Male
                                            </label>
                                        </div>
                                        <div className="form-check">
                                            <input className="form-check-input" type="radio" name="gender" id="femal" value="2" onChange={this.handleChange.bind(this)} checked={this.state.userInfo.gender === '2'}/>
                            ...
                            <div className="form-group">
                                <label>City</label>
                                <Dropdown placeholder='Select City' name="city"   fluid search selection options={cityOptions}  onChange={this.handleChangeDropDown} value={this.state.userInfo.city}/>
                                <span className="invalid-feedback rtl" style={{ display : errors["city"] ? 'block' : 'none'}}>{errors["city"]}</span>
                            </div>
                            <div className="form-group">
                                <label>Bio</label>
                                <textarea type="text" id="bio" name="bio" placeholder='Tell us more about you...' className={["md-textarea" , errors["bio"] ? 'is-invalid' : ''].join(' ')} style={{height: 70}} onChange={this.handleChange.bind(this)}>{this.state.userInfo.bio}</textarea>
                                <span className="invalid-feedback rtl" style={{ display : errors["bio"] ? 'block' : 'none'}}>{errors["bio"]}</span>
                            </div>
                            <div className="text-center">
                                <button className="btn btn-deep-orange">Edit & save</button>
                            </div>
                        </form>
                    </div>
                </div>
